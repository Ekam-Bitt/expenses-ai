  ...
  .superRefine((env, ctx) => {
    ...
    if (
      ...
      ctx.addIssue({
        code: ZodIssueCode.custom,
        message:
          'If NEXT_PUBLIC_ENABLE_RECEIPT_EXTRACT or NEXT_PUBLIC_ENABLE_CATEGORY_EXTRACT is specified, then either OPENAI_API_KEY or GEMINI_API_KEY must be specified too',
      })
    }
  })
